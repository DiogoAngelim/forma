...
import { AppError, type GeneratedOutput } from '@forma/shared';
...
type ExtractedAsset = {
  id: string;
  kind: 'image' | 'svg' | 'font' | 'video' | 'audio' | 'document' | 'asset';
  source: 'local' | 'remote';
  path?: string;
  url?: string;
  mime?: string;
...
export function selectHtmlEntry(files: ProjectBundleFile[]): ProjectBundleFile {
...
export function selectHtmlEntries(files: ProjectBundleFile[]): ProjectBundleFile[] {
  const htmlFiles = files.filter((file) => file.path.toLowerCase().endsWith('.html'));
...
export function normalizeConverterOutput(
  raw: unknown,
  sourceHtml: string,
  blockIdentity?: { title: string; slug: string; namespace?: string }
  ...
  if (raw && typeof raw === 'object') {
    ...
    const value = raw as { blocks?: unknown; html?: unknown; markup?: unknown; content?: unknown };
...
export async function generateGutenberg(input: GenerateGutenbergInput): Promise<GeneratedOutput> {
  ...
  try {
    ...
    const metadataArtifact = await input.artifactWriter.putObject(
...
async function generateMultiPageGutenberg(input: GenerateGutenbergInput, entries: ProjectBundleFile[]): Promise<GeneratedOutput> {
  const converter = input.converter ?? (await loadHtmlToGutenberg());
  const warnings: string[] = [];
  const pages: Array<{
    entry: ProjectBundleFile;
    identity: { title: string; slug: string };
...
export async function generatePluginZipBuffer(input: {
  ...
  uploadId: string;
  converter?: HtmlToGutenbergConverter;
  assets?: Array<{ path: string; mime: string; size: number }>;
}): Promise<Buffer> {
  const converter = input.converter ?? (await loadHtmlToGutenberg());
  const tempDir = await mkdtemp(join(tmpdir(), 'forma-gutenberg-'));
...
async function loadHtmlToGutenberg(): Promise<HtmlToGutenbergConverter> {
  const mod = (await import('html-to-gutenberg') as unknown) as {
    default?: HtmlToGutenbergConverter;
    convert?: HtmlToGutenbergConverter;
  };
...
async function createPluginZipArtifact(
  tempDir: string,
  projectId: string,
  sessionId: string,
  artifactWriter: ArtifactWriter,
  sourceFiles: ProjectBundleFile[] = [],
...
async function createMultiBlockPluginZipArtifact(
  blocks: Array<Record<string, unknown>>,
  projectId: string,
  sessionId: string,
  artifactWriter: ArtifactWriter,
...
async function convertHtmlToJsx(html: string): Promise<string | null> {
  try {
    const packageName = 'node-html-to-jsx';
    const mod = await import(packageName) as Record<string, unknown>;
    const converter = (mod.default ?? mod.HTMLtoJSX ?? mod) as any;
...
function addMultiBlockPluginFiles(zip: JSZip, blocks: Array<Record<string, unknown>>): void {
  const usedFolders = new Set<string>();
  const usedBlockSlugs = new Set<string>();
...
function normalizePageBlock(
  normalized: NormalizedConverterOutput,
  entry: ProjectBundleFile,
  blockIdentity: { title: string; slug: string },
  pageIndex: number
): GutenbergBlock {
...
function blockHtmlForPlugin(block: Record<string, unknown>): string {
...
function buildMultiBlockPluginIndexPhp(descriptors: Array<{ folder: string }>): string {
...
async function addFilesToZip(dirPath: string, zipFolder: JSZip): Promise<void> {
...
function toGeneratedFiles(value: unknown): Record<string, string> | undefined {
  if (!value || typeof value !== 'object' || Array.isArray(value)) {
    return undefined;
  }

  const entries = Object.entries(value as Record<string, unknown>).filter(
    ([name, contents]) => typeof contents === 'string' && isGeneratedFileName(name)
...
function extractBlocksFromGeneratedFiles(files: Record<string, string>, sourceHtml: string): GutenbergBlock[] {
...
function inferBlockIdentity(entryPath: string): { title: string; slug: string } {
...
async function writeGeneratedFileArtifacts(
  baseKey: string,
  generatedFiles: Record<string, string> | undefined,
  artifactWriter: ArtifactWriter
): Promise<Array<{ name: string; key: string; url: string; contentType: string; size: number }>> {
  if (!generatedFiles) {
    return [];
  }

  const artifacts: Array<{ name: string; key: string; url: string; contentType: string; size: number }> = [];
...
function contentTypeForGeneratedFile(name: string): string {
...
function ensureGutenbergMarkup(markup: string): string {
...
function containsGutenbergBlock(markup: string): boolean {
  return /<!--\s*wp:[\s\S]*?-->/g.test(markup);
}

function looksLikeGeneratedGutenbergJsx(markup: string): boolean {
  return /<RichText\b|<MediaUpload\b|value=\{attributes\.|onChange=\s*\(/.test(markup);
}

function normalizeHtmlAttributes(markup: string): string {
  return markup
    .replace(/\bclassName=/g, 'class=')
    .replace(/\bhtmlFor=/g, 'for=');
}

function normalizeBlock(block: unknown, index: number): GutenbergBlock {
  if (block && typeof block === 'object') {
    return {
      id: String((block as { id?: unknown }).id ?? `block-${index + 1}`),
      name: String((block as { name?: unknown }).name ?? (block as { type?: unknown }).type ?? 'core/group'),
...
function extractBlocksFromMarkup(markup: string): GutenbergBlock[] {
...
function ensureUsableGeneratedFiles(
  generatedFiles: Record<string, string>,
  sourceHtml: string,
  blockIdentity?: { title: string; slug: string; namespace?: string }
...
function sanitizeGeneratedFiles(generatedFiles: Record<string, string>): Record<string, string> {
...
function isSchemaOnlyScript(value: unknown): value is string {
  if (typeof value !== 'string') return false;
  const trimmed = value.trim();
  if (!trimmed.startsWith('{') || !trimmed.endsWith('}')) return false;

  try {
    const parsed = JSON.parse(trimmed) as { '@context'?: unknown; '@type'?: unknown };
...
function isUsableBlockJs(value: unknown): value is string {
  return typeof value === 'string' && value.includes('registerBlockType') && value.trim().length > 0;
}

function buildGeneratedBlockMarkup(
  generatedFiles: Record<string, string>,
  sourceHtml: string,
  blockIdentity?: { title: string; slug: string; namespace?: string }
): string {
...
function extractBlockName(blockJs: string | undefined): string | undefined {
  return extractBlockNames(blockJs)[0];
}

function extractBlockNames(blockJs: string | undefined): string[] {
  if (!blockJs) return [];

  return [...blockJs.matchAll(/registerBlockType\(\s*['"]([^'"]+)['"]/g)].map((match) => match[1]).filter(Boolean) as string[];
}

function fallbackBlockName(blockIdentity?: { slug: string; namespace?: string }): string {
  if (!blockIdentity) return 'core/html';

  return `${slugifyBlockSegment(blockIdentity.namespace ?? 'wp')}/${slugifyBlockSegment(blockIdentity.slug)}`;
}

function slugifyBlockSegment(value: string): string {
  const slug = slugify(value).replace(/-/g, '');
  return /^[a-z]/.test(slug) ? slug : `block${slug}`;
}

function buildFallbackBlockJs(sourceHtml: string, blockIdentity: { title: string; slug: string; namespace?: string }): string {
...
function buildFallbackIndexPhp(blockIdentity: { title: string; slug: string; namespace?: string }): string {
  const blockName = fallbackBlockName(blockIdentity);
...
function splitHtmlIntoBlockSections(sourceHtml: string, blockIdentity?: { title: string; slug: string; namespace?: string }): BlockSection[] {
...
function collectElementSections(html: string, tagNames: string[]): Array<{ tagName: string; html: string }> {
  const tagPattern = tagNames.join('|');
  const openingTagRegex = new RegExp(`<(${tagPattern})\\b[^>]*>`, 'gi');
  const sections: Array<{ start: number; end: number; tagName: string; html: string }> = [];

  for (const match of html.matchAll(openingTagRegex)) {
    const start = match.index ?? 0;
    if (sections.some((section) => start >= section.start && start < section.end)) continue;
...
function collectImmediateChildSections(html: string): Array<{ tagName: string; html: string }> {
  const sections: Array<{ tagName: string; html: string }> = [];
  let cursor = 0;
...
function findMatchingElementEnd(html: string, tagName: string, fromIndex: number): number | null {
...
function inferSectionTitle(html: string, tagName: string, blockIdentity: { title: string } | undefined, index: number, total: number): string {
  ...
  if (total === 1) return blockIdentity?.title ?? title;
  return `${blockIdentity?.title ?? 'Generated'} ${index + 1}: ${title}`;
}

function sectionBlockName(baseName: string, title: string, index: number): string {
...
function firstElementTagName(html: string): string | null {
  return html.match(/<([a-z][\w:-]*)\b/i)?.[1]?.toLowerCase() ?? null;
}

function extractOpeningTagAttributes(html: string): Record<string, string> {
...
function extractExecutableScriptContents(html: string): string {
...
function stripDangerousDocumentScripts(html: string): string {
  return html.replace(/<script\b[^>]*type\s*=\s*["']application\/ld\+json["'][^>]*>[\s\S]*?<\/script>/gi, '');
}

async function writeGeneratedFilesToDirectory(tempDir: string, generatedFiles: Record<string, string>): Promise<void> {
...
function addSourceAssetsToZip(zip: JSZip, files: ProjectBundleFile[], htmlEntryPath?: string): void {
  const htmlDir = htmlEntryPath ? dirname(htmlEntryPath) : '.';
  const placeUnderAssets = !htmlEntryPath;
  const usedNames = new Set<string>();

  for (const file of files) {
    if (file.path.toLowerCase().endsWith('.html') || isGeneratedFileName(basename(file.path))) continue;
    if (!isBundleAsset(file)) continue;
...
function extractStyleTokens(files: ProjectBundleFile[], sourceHtml: string): Record<string, unknown> {
  const styleSources = [
...
function extractAssets(files: ProjectBundleFile[], sourceHtml: string): ExtractedAsset[] {
  const localAssets: ExtractedAsset[] = files
  ...
  const remoteAssets: ExtractedAsset[] = extractRemoteAssetUrls(sourceHtml).map((url) => ({
    id: `remote:${url}`,
    kind: assetKind(url),
...
function extractRemoteAssetUrls(html: string): string[] {
...
function addAssetUrl(urls: Set<string>, value: string | undefined): void {
...
function isStyleSourceFile(file: ProjectBundleFile): boolean {
...
function uniqueDeclarationValues(value: string, property: string): string[] {
...
function extractTailwindTokens(source: string): { colors: string[]; fontFamilies: string[]; fontSizes: string[] } {
  if (!source.trim()) {
...
function extractObjectLikeValues(source: string, keys: string[]): string[] {
  const values: string[] = [];
...
function extractObjectLikeSection(source: string, key: string): string | null {
...
function safeText(content: Buffer): string {
  return content.toString('utf8');
}

function assetKind(path: string, mime = ''): ExtractedAsset['kind'] {
  if (mime === 'image/svg+xml' || /\.svg(?:[?#].*)?$/i.test(path)) return 'svg';
  if (mime.startsWith('image/') || /\.(png|jpe?g|gif|webp|avif|ico)(?:[?#].*)?$/i.test(path)) return 'image';
  if (mime.startsWith('font/') || /\.(woff2?|ttf|otf|eot)(?:[?#].*)?$/i.test(path)) return 'font';
  if (mime.startsWith('video/') || /\.(mp4|webm|mov|m4v)(?:[?#].*)?$/i.test(path)) return 'video';
  if (mime.startsWith('audio/') || /\.(mp3|wav|ogg|m4a)(?:[?#].*)?$/i.test(path)) return 'audio';
...
function isBundleAsset(file: ProjectBundleFile): boolean {
