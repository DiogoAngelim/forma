...
import { generateGutenberg, generateMultiBlockPluginZipBuffer, generatePluginZipBuffer, normalizeConverterOutput, selectHtmlEntries, selectHtmlEntry } from './index.js';
const writer = {
    async putObject(key) {
...
describe('gutenberg generator', () => {
    ...
    it('normalizes object and fallback converter outputs', () => {
        expect(normalizeConverterOutput({ html: '<p className="lead">Fallback</p>' }, '<p>Source</p>').markup).toBe('<!-- wp:html -->\n<p class="lead">Fallback</p>\n<!-- /wp:html -->');
        expect(normalizeConverterOutput({ html: '<p>Fallback</p>' }, '<p>Source</p>').blocks[0]?.name).toBe('core/html');
        expect(normalizeConverterOutput({ blocks: [] }, '<p>Source</p>').markup).toBe('<!-- wp:html -->\n<p>Source</p>\n<!-- /wp:html -->');
        ...
        expect(normalizeConverterOutput(null, '<main>Source</main>').markup).toBe('<!-- wp:html -->\n<main>Source</main>\n<!-- /wp:html -->');
    });
    it('normalizes generated converter file maps into block metadata', () => {
        ...
        }, '<section class="team">Team</section>', { title: 'Grid', slug: 'grid-with-large-round-images', namespace: 'wp' });
    ...
    it('keeps semantic html sections in a single generated block', () => {
        ...
        }, '<header class="hero"><h1>Hero</h1></header><section><h2>Features</h2></section><footer>Bye</footer>', { title: 'Landing Page', slug: 'landing-page', namespace: 'wp' });
    ...
    it('synthesizes a usable block when the converter returns an incomplete file map', () => {
        const normalized = normalizeConverterOutput({
            ...
            'style.css': '.team{}'
        }, '<section className="team"><h2>Team</h2></section>', { title: 'Team Grid', slug: 'team-grid', namespace: 'wp' });
    ...
    it('preserves uploaded html order as one block per page', async () => {
        ...
        const zipBody = stored.find((item) => item.key.endsWith('/plugin/gutenberg-plugin.zip'))?.body;
    ...
    it('stores generated converter files as first-class artifacts', async () => {
        ...
        expect(result.metadata.generatedFiles).toEqual(expect.arrayContaining([expect.objectContaining({ name: 'block.js', contentType: 'application/javascript' })]));
        expect(result.metadata.generatedFiles).toEqual(expect.arrayContaining([expect.objectContaining({ name: 'index.php', contentType: 'application/x-httpd-php' })]));
