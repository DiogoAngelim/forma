...
import type { AiSuggestion, AnalysisResult, GeneratedOutput, SuggestionAction } from '@forma/shared';
...
type BlockCandidate = Record<string, unknown> & {
  id: string;
  kind: string;
  selector: string;
  label: string;
...
export function analyzeGeneratedOutput(input: AnalysisInput): AnalysisResult {
...
export function generateDeterministicSuggestions(projectId: string, analysis: AnalysisResult): AiSuggestion[] {
...
export function applySuggestionToOutput(output: GeneratedOutput, suggestion: Pick<AiSuggestion, 'action'>): GeneratedOutput {
  const action = suggestion.action;
  const metadata = {
    ...output.metadata,
    appliedActions: [...((output.metadata.appliedActions as unknown[]) ?? []), action]
  };
...
function injectFormaStyle(markup: string, actionType: SuggestionAction['type']): string {
...
function findBySelector($: cheerio.CheerioAPI, selector: string, kind: string): BlockCandidate[] {
  return $(selector)
    .toArray()
    .map((element, index) => candidate($, element, kind, `${kind}-${index + 1}`));
}

function findSemanticSections($: cheerio.CheerioAPI, terms: string[], kind: string): BlockCandidate[] {
  const selector = terms.map((term) => `[class*="${term}"],[id*="${term}"],[data-block*="${term}"]`).join(',');
  return findBySelector($, selector, kind);
}

function findRepeatedSections($: cheerio.CheerioAPI): BlockCandidate[] {
...
function candidate($: cheerio.CheerioAPI, element: Parameters<cheerio.CheerioAPI>[0], kind: string, id: string): BlockCandidate {
  const node = $(element);
  const raw = element as { tagName?: string } | undefined;
...
function selectorFor(element: Parameters<cheerio.CheerioAPI>[0]): string {
  const node = element as { attribs?: Record<string, string>; tagName?: string };
...
function extractTypography(css: string): Record<string, unknown> {
  ...
  root.walkDecls((decl) => {
    if (decl.prop === 'font-size') sizes.add(decl.value);
    if (decl.prop === 'font-family') families.add(decl.value);
  });
...
function extractColors(css: string): Record<string, unknown> {
...
function idsOf(items: Array<Record<string, unknown>>): string[] {
