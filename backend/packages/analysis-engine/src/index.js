...
export function applySuggestionToOutput(output, suggestion) {
    ...
    if (action.type === 'normalize_typography' || action.type === 'rewrite_styles') {
        return {
            ...output,
            metadata,
            markup: `${output.markup}\n<!-- forma:${action.type} ${JSON.stringify(action.parameters)} -->`
        };
    }
...
function candidate($, element, kind, id) {
    const node = $(element);
    const label = node.attr('aria-label') ?? node.attr('id') ?? node.attr('class') ?? element.tagName;
    return {
        id,
        kind,
...
function selectorFor(element) {
    ...
    const className = element.attribs?.class?.split(/\s+/)[0];
    return id ? `#${id}` : className ? `${element.tagName}.${className}` : element.tagName;
}
