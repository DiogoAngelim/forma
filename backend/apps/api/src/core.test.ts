...
describe('api core modules', () => {
  ...
  it('builds previews and export artifacts', async () => {
    ...
    const pluginZip = await buildExportArtifact(
      { ...output, metadata: { ...output.metadata, pluginZipUrl: 'https://cdn.example/plugin.zip' } },
      { type: 'plugin_zip' }
    );

    expect(pluginZip.contentType).toBe('application/zip');
    const zip = await JSZip.loadAsync(pluginZip.body as Buffer);
  ...
  it('extracts only structured zip bundles', async () => {
    ...
    for (const path of mimes) rich.file(path, '');
