...
export async function extractZipBundle(buffer: Buffer): Promise<ProjectBundleFile[]> {
  const zip = await JSZip.loadAsync(buffer);
  const files: ProjectBundleFile[] = [];

  for (const [path, entry] of Object.entries(zip.files)) {
    if (entry.dir || ignored.some((part) => path.includes(part))) continue;
...
export function assertZipUpload(filename: string | undefined, mime: string | undefined): void {
...
function mimeForPath(path: string): string {
  ...
  if (lower.endsWith('tailwind.config.js') || lower.endsWith('tailwind.config.ts')) return 'application/javascript';
