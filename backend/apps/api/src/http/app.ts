...
import { AppError, exportRequestSchema, type GeneratedOutput } from '@forma/shared';
...
export async function buildApp(deps: BuildAppDeps): Promise<FastifyInstance> {
  const app = Fastify({ logger: { level: deps.env.LOG_LEVEL } }).withTypeProvider<ZodTypeProvider>();
  ...
  app.get('/projects/*', async (request: any, reply) => {
  ...
  async function handleRegister(request: any, reply: any) {
  ...
  async function handleLogin(request: any, reply: any) {
    ...
    if (!user || !user.password_hash || !verifyPassword(password, user.password_hash)) throw new AppError(401, 'invalid_credentials', 'Invalid email or password');
  ...
  async function handleListProjects(request: any) {
    ...
    return Promise.all(projects.map(async (project) => {
      ...
      const projectMetadata = parseJson<Record<string, unknown>>(project.metadata, {});
  ...
  async function handleCreateProject(request: any, reply: any) {
    const id = randomUUID();
    await deps.db
      .insertInto('projects')
  ...
  app.get('/public/:slug', { schema: { params: z.object({ slug: z.string() }) } }, async (request: any) => {
    const project = await deps.db
      ...
      .executeTakeFirst();
    if (!project) throw new AppError(404, 'public_project_not_found', 'Public project not found');
    const metadata = parseJson<Record<string, unknown>>(project.metadata, {});
    return {
      id: project.id,
  ...
  await app.register(async (webhookApp) => {
    ...
    webhookApp.post('/billing/webhook', async (request: any, reply) => {
  ...
  await app.register(async (apiApp) => {
    apiApp.get('/me', { preHandler: [app.authenticate] }, async (request: any) => {
    ...
    apiApp.delete('/me', { preHandler: [app.authenticate] }, async (request: any, reply) => {
    ...
    apiApp.get('/billing/subscription', { preHandler: [app.authenticate] }, async (request: any) => {
      return billingStatusForUser(deps.db, request.user.sub);
    });

    apiApp.post('/billing/checkout', { preHandler: [app.authenticate], schema: { body: billingCheckoutSchema } }, async (request: any) => {
      ...
      if (!plan) throw new AppError(400, 'invalid_plan', 'Unknown billing plan');
    ...
    apiApp.post('/billing/portal', { preHandler: [app.authenticate] }, async (request: any) => {
      if (!stripe) throw new AppError(503, 'stripe_not_configured', 'Stripe is not configured');
    ...
    apiApp.get('/projects/:id', { preHandler: [app.authenticate], schema: { params: paramsIdSchema } }, async (request: any) => {
    ...
    apiApp.patch('/projects/:id', { preHandler: [app.authenticate], schema: { params: paramsIdSchema, body: projectUpdateSchema } }, async (request: any) => {
      const project = await deps.db
        .selectFrom('projects')
      ...
      const projectMetadata = metadataPatch
        ? { ...parseJson<Record<string, unknown>>(project.metadata, {}), ...metadataPatch }
        : null;
      const updateValues: Record<string, unknown> = { updated_at: now };
      if (request.body.name !== undefined) updateValues.name = request.body.name;
      if (projectMetadata) updateValues.metadata = JSON.stringify(projectMetadata);
      ...
      if (publicProject) {
        ...
        if (request.body.name !== undefined) publicValues.title = request.body.name;
        if (metadataPatch) {
          publicValues.metadata = JSON.stringify({
            ...parseJson<Record<string, unknown>>(publicProject.metadata, {}),
            ...metadataPatch
          });
        }
    ...
    apiApp.delete('/projects', { preHandler: [app.authenticate], schema: { body: projectBatchDeleteSchema } }, async (request: any, reply) => {
      await deleteOwnedProjects(deps.db, request.body.ids, request.user.sub);
      return reply.status(204).send();
    });

    apiApp.post('/projects/:id/duplicate', { preHandler: [app.authenticate], schema: { params: paramsIdSchema } }, async (request: any, reply) => {
      const project = await deps.db.selectFrom('projects').selectAll().where('id', '=', request.params.id).where('user_id', '=', request.user.sub).executeTakeFirst();
      if (!project) throw new AppError(404, 'project_not_found', 'Project not found');
    ...
    apiApp.delete('/projects/:id', { preHandler: [app.authenticate], schema: { params: paramsIdSchema } }, async (request: any, reply) => {
      await deleteOwnedProjects(deps.db, [request.params.id], request.user.sub, { requireAll: true });
      return reply.status(204).send();
    });

    apiApp.get('/projects/:id/output', { preHandler: [app.authenticate], schema: { params: paramsIdSchema } }, async (request: any) => {
      ...
      if (!output) throw new AppError(404, 'output_not_found', 'Upload and convert a project before loading output');
      return output;
    });

    apiApp.patch('/projects/:id/output', { preHandler: [app.authenticate], schema: { params: paramsIdSchema, body: outputUpdateSchema } }, async (request: any) => {
      await assertProjectOwner(deps.db, request.params.id, request.user.sub);
      const output = await store.latestOutput(request.params.id);
      if (!output) throw new AppError(404, 'output_not_found', 'Upload and convert a project before updating output');
    ...
    apiApp.get('/projects/:id/source-files', { preHandler: [app.authenticate], schema: { params: paramsIdSchema } }, async (request: any) => {
      await assertProjectOwner(deps.db, request.params.id, request.user.sub);
    ...
    apiApp.get('/projects/:id/source-file', { preHandler: [app.authenticate], schema: { params: paramsIdSchema, querystring: z.object({ path: z.string().min(1) }) } }, async (request: any, reply) => {
      await assertProjectOwner(deps.db, request.params.id, request.user.sub);
      const upload = await deps.db
        .selectFrom('uploads')
        .select(['manifest'])
      ...
      if (!upload) throw new AppError(404, 'source_files_not_found', 'No source files found for project');

      const sourceFile = parseManifestFiles(upload.manifest).find((file) => file.path === request.query.path);
      if (!sourceFile?.r2Key) throw new AppError(404, 'source_file_not_found', 'Source file not found');
    ...
    apiApp.post('/projects/:id/source-files', { preHandler: [app.authenticate], schema: { params: paramsIdSchema } }, async (request: any) => {
      await assertProjectOwner(deps.db, request.params.id, request.user.sub);
      ...
      const storedFiles = await Promise.all(files.map(async (file) => {
        const stored = await deps.storage.putObject(
          `projects/${request.params.id}/uploads/${uploadId}/source/${file.path}`,
          file.content,
          file.mime
        );
        return { path: file.path, mime: file.mime, size: file.size, r2Key: stored.key };
      }));
      const existingManifest = parseJson<Record<string, unknown>>(existingUpload?.manifest, {});
      ...
      await syncSupplementalSourcesToLatestOutput(deps.db, request.params.id, mergedFiles, storedFiles);
    ...
    apiApp.get('/projects/:id/preview', { preHandler: [app.authenticate], schema: { params: paramsIdSchema } }, async (request: any, reply) => {
      await assertProjectOwner(deps.db, request.params.id, request.user.sub);
      const project = await deps.db.selectFrom('projects').select(['metadata']).where('id', '=', request.params.id).executeTakeFirst();
      const output = await store.latestOutput(request.params.id);
      if (!output) throw new AppError(404, 'output_not_found', 'Upload and convert a project before previewing');
      const metadata = parseJson<Record<string, unknown>>(project?.metadata, {});
      ...
      return reply.type('text/html').send(buildIframePreview(output, {
        ...
        styles,
        remoteStyles: mergeUrlLists(output.metadata.remoteStyles, metadata.remoteStyles),
        remoteScripts: mergeUrlLists(output.metadata.remoteScripts, metadata.remoteScripts)
      }));
    });

    apiApp.post('/projects/:id/upload', { preHandler: [app.authenticate], schema: { params: paramsIdSchema } }, async (request: any, reply) => {
      await assertProjectOwner(deps.db, request.params.id, request.user.sub);
    ...
    apiApp.post('/projects/:id/import/url', { preHandler: [app.authenticate], schema: { params: paramsIdSchema, body: urlImportSchema } }, async (request: any, reply) => {
      await assertProjectOwner(deps.db, request.params.id, request.user.sub);
      ...
      const storedSource = await deps.storage.putObject(
        `projects/${request.params.id}/uploads/${uploadId}/source/${htmlPath}`,
        html,
        'text/html'
      );
      const files = [
    ...
    apiApp.post('/projects/:id/import/github', { preHandler: [app.authenticate], schema: { params: paramsIdSchema } }, async (request: any) => {
      await assertProjectOwner(deps.db, request.params.id, request.user.sub);
    ...
    apiApp.post('/projects/:id/convert', { preHandler: [app.authenticate], schema: { params: paramsIdSchema } }, async (request: any) => {
      await assertProjectOwner(deps.db, request.params.id, request.user.sub);
    ...
    apiApp.get('/projects/:id/events', { schema: { params: paramsIdSchema } }, async (request: any, reply) => {
    ...
    apiApp.get('/projects/:id/suggestions', { preHandler: [app.authenticate], schema: { params: paramsIdSchema } }, async (request: any) => {
      await assertProjectOwner(deps.db, request.params.id, request.user.sub);
      return deps.db.selectFrom('ai_suggestions').selectAll().where('project_id', '=', request.params.id).orderBy('created_at', 'desc').execute();
    });

    apiApp.post('/projects/:id/suggestions/:suggestionId/apply', { preHandler: [app.authenticate], schema: { params: paramsSuggestionSchema } }, async (request: any) => {
      await assertProjectOwner(deps.db, request.params.id, request.user.sub);
    ...
    apiApp.post('/projects/:id/export', { preHandler: [app.authenticate], schema: { params: paramsIdSchema, body: exportRequestSchema } }, async (request: any) => {
      await assertProjectOwner(deps.db, request.params.id, request.user.sub);
      return exportProject({ store, storage: deps.storage, events, ai }, { projectId: request.params.id, request: request.body });
    });

    apiApp.post('/projects/:id/publish', { preHandler: [app.authenticate], schema: { params: paramsIdSchema, body: z.object({ slug: z.string().min(3), title: z.string().min(1) }) } }, async (request: any) => {
      const project = await deps.db
        .selectFrom('projects')
        .selectAll()
        .where('id', '=', request.params.id)
      ...
      const output = await store.latestOutput(request.params.id);
      if (!output) throw new AppError(409, 'missing_output', 'Generate Gutenberg output before publishing');
      ...
      const now = new Date().toISOString();
      const projectMetadata = parseJson<Record<string, unknown>>(project.metadata, {});
      const existingMetadata = parseJson<Record<string, unknown>>(existingPublicProject?.metadata, {});
    ...
    apiApp.post('/projects/:id/unpublish', { preHandler: [app.authenticate], schema: { params: paramsIdSchema } }, async (request: any) => {
      await assertProjectOwner(deps.db, request.params.id, request.user.sub);
      const now = new Date().toISOString();
    ...
    apiApp.post('/projects/:id/versions', { preHandler: [app.authenticate], schema: { params: paramsIdSchema, body: z.object({ label: z.string().min(1) }) } }, async (request: any) => {
      await assertProjectOwner(deps.db, request.params.id, request.user.sub);
      const output = await store.latestOutput(request.params.id);
      if (!output) throw new AppError(409, 'missing_output', 'Generate Gutenberg output before snapshotting');
      const id = randomUUID();
    ...
    apiApp.post('/projects/:id/versions/:versionId/restore', { preHandler: [app.authenticate], schema: { params: z.object({ id: z.string().uuid(), versionId: z.string().uuid() }) } }, async (request: any) => {
      await assertProjectOwner(deps.db, request.params.id, request.user.sub);
      ...
      if (!version) throw new AppError(404, 'version_not_found', 'Project version not found');
    ...
    apiApp.get('/showcase', async () => {
      const rows = await deps.db
        ...
        .execute();
      return rows.map((row) => {
        const output = parseJson(row.after_snapshot, {}) as { blocks?: unknown[]; metadata?: Record<string, unknown> };
    ...
    apiApp.get('/public/:slug', { schema: { params: z.object({ slug: z.string() }) } }, async (request: any) => {
      const project = await deps.db
        .selectFrom('public_projects')
        .innerJoin('projects', 'projects.id', 'public_projects.project_id')
        .innerJoin('users', 'users.id', 'projects.user_id')
    ...
    apiApp.post('/public/:slug/reaction', { schema: { params: z.object({ slug: z.string() }), body: publicReactionSchema } }, async (request: any) => {
...
async function readMultipartProjectBundle(request: any, uploadId: string): Promise<MultipartProjectBundle> {
  const parts: UploadedMultipartFile[] = [];

  for await (const part of request.files()) {
    if (!part || part.type === 'field') continue;
...
async function readMultipartLooseFiles(request: any): Promise<ProjectBundleFile[]> {
  const parts: UploadedMultipartFile[] = [];
...
function sanitizeUploadPath(value: string): string {
  const normalized = value.replace(/\\/g, '/').replace(/^\/+/, '');
  const parts = normalized.split('/').filter((part) => part && part !== '.' && part !== '..');
  return parts.join('/') || 'file';
}

function isZipUploadPart(part: UploadedMultipartFile): boolean {
  return part.mimetype.includes('zip') || part.filename.toLowerCase().endsWith('.zip');
}

function dedupeProjectBundlePaths(files: ProjectBundleFile[]): ProjectBundleFile[] {
...
function isSupplementalSourceFile(file: ProjectBundleFile): boolean {
  return /\.(css|scss|sass|less|js|mjs|cjs|ts|tsx)$/i.test(file.path);
}

function isHtmlPath(path: string, mime = ''): boolean {
  return mime.includes('html') || /\.html?$/i.test(path);
}

function publicProjectAuthor(
  row: { author_name?: unknown; author_email?: unknown },
  metadata: Record<string, unknown>
): { name: string; avatar: string | null } {
  const metadataAuthor = metadata.author && typeof metadata.author === 'object'
    ? metadata.author as { name?: unknown; avatar?: unknown }
    : {};
...
function htmlEntryPath(url: URL): string {
  const pathname = url.pathname.endsWith('/') ? `${url.pathname}index.html` : url.pathname;
  const filename = pathname.split('/').filter(Boolean).pop() || 'index.html';
  return filename.includes('.') ? filename : `${filename}.html`;
}

function parseManifestFiles(manifest: unknown): Array<{ path: string; mime?: string; size?: number; r2Key?: string }> {
  const value = parseJson(manifest, {});
  if (!value || typeof value !== 'object' || !Array.isArray((value as { files?: unknown }).files)) {
    return [];
  }

  const files: Array<{ path: string; mime?: string; size?: number; r2Key?: string }> = [];

  for (const file of (value as { files: unknown[] }).files) {
    if (!file || typeof file !== 'object') continue;
    const row = file as { path?: unknown; mime?: unknown; size?: unknown; r2Key?: unknown };
    if (typeof row.path !== 'string') continue;
    const parsedFile: { path: string; mime?: string; size?: number; r2Key?: string } = { path: row.path };
    if (typeof row.mime === 'string') parsedFile.mime = row.mime;
    if (typeof row.size === 'number') parsedFile.size = row.size;
    if (typeof row.r2Key === 'string') parsedFile.r2Key = row.r2Key;
...
function mergeManifestFiles(
  existingFiles: Array<{ path: string; mime?: string; size?: number; r2Key?: string }>,
  addedFiles: Array<{ path: string; mime?: string; size?: number; r2Key?: string }>
): Array<{ path: string; mime?: string; size?: number; r2Key?: string }> {
  const merged = new Map<string, { path: string; mime?: string; size?: number; r2Key?: string }>();
...
async function assertProjectOwner(db: Kysely<Database>, projectId: string, userId: string): Promise<void> {
...
function firstPreviewImageUrl(metadata: Record<string, unknown>): string | null {
  const previewImage = metadata.previewImageUrl ?? metadata.previewImage ?? metadata.thumbnailUrl ?? metadata.thumbnail;
  if (typeof previewImage === 'string' && previewImage.trim()) return previewImage;
  ...
  for (const asset of assets) {
    ...
    const row = asset as { kind?: unknown; url?: unknown };
...
function normalizeProjectMetadataPatch(value: unknown): Record<string, unknown> | null {
  if (!value || typeof value !== 'object') return null;
  const patch = { ...(value as Record<string, unknown>) };

  if ('tags' in patch) patch.tags = normalizeTags(patch.tags).slice(0, 24);
  if ('remoteStyles' in patch) patch.remoteStyles = normalizeUrlList(patch.remoteStyles);
  if ('remoteScripts' in patch) patch.remoteScripts = normalizeUrlList(patch.remoteScripts);

  return patch;
}

function slugifyPublicSlug(value: string): string {
  const slug = value
    .trim()
    .toLowerCase()
    .replace(/[^a-z0-9]+/g, '-')
...
async function uniquePublicSlug(db: Kysely<Database>, desiredSlug: string, existingPublicProjectId?: string): Promise<string> {
...
async function ensureBillingSchema(db: Kysely<Database>): Promise<void> {
  await sql.raw(`
...
async function billingStatusForUser(db: Kysely<Database>, userId: string) {
...
async function handleStripeEvent(db: Kysely<Database>, stripe: Stripe, event: Stripe.Event): Promise<void> {
...
async function syncStripeSubscription(db: Kysely<Database>, subscription: Stripe.Subscription): Promise<void> {
  ...
  await db
    ...
    .values({
      ...
      trial_end: unixToIso((subscription as any).trial_end),
...
function subscriptionIdFromInvoice(invoice: Stripe.Invoice): string | null {
  const candidate = (invoice as any).subscription ?? (invoice as any).parent?.subscription_details?.subscription;
...
async function readGeneratedStyles(output: GeneratedOutput): Promise<string[]> {
  const files = output.metadata.generatedFiles;
  const supplemental = normalizeSupplementalSources(output.metadata.supplementalStyles)
    .map((source) => source.content)
    .filter((content) => content.trim().length > 0);
  if (!Array.isArray(files)) return supplemental;

  const cssKeys = files
    .filter((file): file is { key?: unknown; name?: unknown; url?: unknown; contentType?: unknown } => Boolean(file && typeof file === 'object'))
  ...
  for (const key of cssKeys) {
    ...
    if (!normalizedKey.startsWith('projects/') || normalizedKey.includes('..')) continue;
...
async function syncSupplementalSourcesToLatestOutput(
  db: Kysely<Database>,
  projectId: string,
  allFiles: Array<{ path: string; mime?: string; size?: number; r2Key?: string }>,
  uploadedFiles: Array<{ path: string; mime?: string; size?: number; r2Key?: string }>
): Promise<void> {
  ...
  const metadata = parseJson<Record<string, unknown>>(output.metadata, {});
  const existingStyles = normalizeSupplementalSources(metadata.supplementalStyles);
  const existingScripts = normalizeSupplementalSources(metadata.supplementalScripts);
  const uploadedContent = new Map<string, string>();

  for (const file of uploadedFiles) {
    if (!file.r2Key) continue;
    try {
      const normalizedKey = normalize(file.r2Key);
      if (!normalizedKey.startsWith('projects/') || normalizedKey.includes('..')) continue;
      uploadedContent.set(file.path, await readFile(join(process.cwd(), 'storage', normalizedKey), 'utf8'));
    } catch {
      // Keep the previous metadata if a supplemental file cannot be read immediately.
    }
  }

  const styleSources = mergeSupplementalSources(existingStyles, allFiles.filter(isSupplementalStyleFile), uploadedContent);
  const scriptSources = mergeSupplementalSources(existingScripts, allFiles.filter(isSupplementalScriptFile), uploadedContent);
  ...
  await db
    ...
    .set({
      metadata: JSON.stringify({
        ...metadata,
        supplementalStyles: styleSources,
        supplementalScripts: scriptSources,
        editedAt: new Date().toISOString()
      })
    })
    .where('id', '=', output.id)
...
type SupplementalSource = { path: string; content: string; mime?: string; size?: number; r2Key?: string };

function mergeSupplementalSources(
  existing: SupplementalSource[],
  files: Array<{ path: string; mime?: string; size?: number; r2Key?: string }>,
  uploadedContent: Map<string, string>
) {
  const byPath = new Map(existing.map((source) => [source.path, source]));
  for (const file of files) {
    const source: SupplementalSource = {
      ...byPath.get(file.path),
      path: file.path,
      content: uploadedContent.get(file.path) ?? byPath.get(file.path)?.content ?? ''
    };
    if (file.mime !== undefined) source.mime = file.mime;
    if (file.size !== undefined) source.size = file.size;
    if (file.r2Key !== undefined) source.r2Key = file.r2Key;
    byPath.set(file.path, source);
  }
  return [...byPath.values()].filter((source) => source.content.trim().length > 0);
}

function normalizeSupplementalSources(value: unknown): SupplementalSource[] {
  if (!Array.isArray(value)) return [];
  return value
    .filter((source): source is Record<string, unknown> => Boolean(source && typeof source === 'object'))
    .map((source) => {
      const normalized: SupplementalSource = {
        path: String(source.path ?? ''),
        content: String(source.content ?? '')
      };
      if (typeof source.mime === 'string') normalized.mime = source.mime;
      if (typeof source.size === 'number') normalized.size = source.size;
      if (typeof source.r2Key === 'string') normalized.r2Key = source.r2Key;
      return normalized;
    })
    .filter((source) => source.path && source.content.trim().length > 0);
}

function isSupplementalStyleFile(file: { path: string; mime?: string }) {
  return file.mime?.includes('css') || /\.(css|scss|sass|less)$/i.test(file.path);
}

function isSupplementalScriptFile(file: { path: string; mime?: string }) {
  return file.mime?.includes('javascript') || /\.(js|mjs|cjs|ts|tsx)$/i.test(file.path);
}

function mergeUrlLists(...values: unknown[]): string[] {
  return [...new Set(values.flatMap((value) => normalizeUrlList(value)))];
}
